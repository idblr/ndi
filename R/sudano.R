...
#'
#' \emph{LQ} is some measure of relative racial homogeneity of each smaller geography within a larger geography. \emph{LQ} can range in value from 0 to infinity because it is ratio of two proportions in which the numerator is the proportion of subgroup population in a smaller geography and the denominator is the proportion of subgroup population in its larger geography. For example, a smaller geography with an \emph{LQ} of 5 means that the proportion of the subgroup population living in the smaller geography is five times the proportion of the subgroup population in its larger geography.
#'
...
#'
#' \describe{
#' \item{\code{lq}}{An object of class 'tbl' for the GEOID, name, and \emph{LQ} at specified smaller census geographies.}
...
sudano <- function(geo_large = 'county',
  ...
  if (geo_large == 'cbsa') {
    ...
    win_cbsa <- sf::st_within(lq_data, dat_cbsa)
    lq_data <- lq_data %>%
      dplyr::mutate(
  ...
  if (geo_large == 'csa') {
    ...
    win_csa <- sf::st_within(lq_data, dat_csa)
    lq_data <- lq_data %>%
      dplyr::mutate(
  ...
  if (geo_large == 'metro') {
    ...
    win_metro <- sf::st_within(lq_data, dat_metro)
    lq_data <- lq_data %>%
      dplyr::mutate(
  ...
  if (geo_large == 'cbsa') {
    lq <- lq_data %>%
      ...
      dplyr::left_join(lq, ., by = dplyr::join_by(GEOID)) %>%
