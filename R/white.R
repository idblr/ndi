...
#' 
#' \emph{V} removes the asymmetry from the Isolation Index (Bell) by controlling for the effect of population composition. The Isolation Index (Bell) is some measure of the probability that a member of one subgroup(s) will meet or interact with a member of another subgroup(s) with higher values signifying higher probability of interaction (less isolation). \emph{V} can range in value from -Inf to Inf.
#' 
...
white <- function(geo_large = 'county',
  ...
  if (geo_large == 'cbsa') {
    ...
    win_cbsa <- sf::st_within(v_data, dat_cbsa)
    v_data <- v_data %>%
      dplyr::mutate(
  ...
  if (geo_large == 'csa') {
    ...
    win_csa <- sf::st_within(v_data, dat_csa)
    v_data <- v_data %>%
      dplyr::mutate(
  ...
  if (geo_large == 'metro') {
    ...
    win_metro <- sf::st_within(v_data, dat_metro)
    v_data <- v_data %>%
      dplyr::mutate(
