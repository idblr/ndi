...
duncan <- function(geo_large = 'county',
    ...
    di_data <- suppressMessages(suppressWarnings(
      tidycensus::get_acs(
        geography = geo_small,
        year = year,
        output = 'wide',
    ...
    if (geo_large == 'cbsa') {
      ...
      win_cbsa <- sf::st_within(di_data, dat_cbsa)
      di_data <- di_data %>%
        dplyr::mutate(
    ...
    if (geo_large == 'csa') {
      ...
      win_csa <- sf::st_within(di_data, dat_csa)
      di_data <- di_data %>%
        dplyr::mutate(
    ...
    if (geo_large == 'metro') {
      ...
      win_metro <- sf::st_within(di_data, dat_metro)
      di_data <- di_data %>%
        dplyr::mutate(
    ...
    if (geo_large == 'state') {
      di <- di_data %>%
        dplyr::left_join(DItmp, by = dplyr::join_by(oid)) %>%
    ...
    out <- list(di = di, di_data = di_data, missing = missingYN)
